...
RSpec.describe MenuItem, type: :model do
  ...
  it "is invalid with length of description more than 150" do
    item = FactoryBot.build(:menu_item, description: 'a'*151)
    item.valid?
    expect(item.errors[:description]).to include("is too long (maximum is 150 characters)")
  end

  it "is invalid without name" do
    item = FactoryBot.build(:menu_item, category:nil)
    item.valid?
